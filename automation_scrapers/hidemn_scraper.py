...
def _solve_challenge_and_get_creds(sb: BaseCase, url: str, verbose: bool, turnstile_delay: float = 0) -> dict:
    ...
    """
    if verbose:
        print(f"\n[INFO] Hide.mn: Using browser to access {url}...")
    ...
    try:
        if turnstile.is_turnstile_present(sb):
            ...
            if verbose:
                print("\n[INFO] Hide.mn: Cloudflare challenge detected. Attempting to solve...")
        ...
        # sb.wait_for_element_present(selector='.table_block > table:nth-child(1)', timeout=20)
            try:
                ...
                if verbose:
                    print("\n[SUCCESS] Hide.mn: First challenge solving failed, trying alternative method.")
                if turnstile_delay > 0:
                    if verbose:
                        print(f"\n[INFO] Hide.mn: --turnstile-delay present. Waiting {turnstile_delay} seconds for Turnstile to load...")
                    time.sleep(turnstile_delay)
                    sb.uc_gui_handle_cf()
                if verbose:
                    print("\n[INFO] Hide.mn: Waiting for the table element...")  
                sb.wait_for_element_present(selector='.table_block > table:nth-child(1)', timeout=20)
        else:
            raise Exception("[ERROR] Hide.mn: No Cloudflare challenge detected.")
        if verbose:
            print("\n[SUCCESS] Hide.mn: Challenge solved or bypassed. Table is present.")
        ...
        if verbose:
            print(f"\n[ERROR] Hide.mn: Failed to solve challenge or extract credentials: {e}")
        return {}
...
def scrape_from_hidemn(sb: BaseCase, verbose: bool = True, turnstile_delay: float = 0) -> List[str]:
    ...
    """
    if verbose:
        print("\n[RUNNING] 'Hide.mn' automation scraper has started.")
    ...
    try:
        ...
        if not creds:
            print("\n[ERROR] Hide.mn: Could not get initial Cloudflare credentials. Aborting.")
            return []
        ...
        if verbose:
            print(f"\n[INFO]   ... Hide.mn: Found {len(initial_proxies)} proxies on first page. Total unique: {len(all_proxies)}.")

        offset = 64
        while True:
            url = URL_TEMPLATE.format(offset=offset)
            if verbose:
                print(f"\n[INFO] Hide.mn: Making direct request to page with offset {offset}...")
            ...
            try:
                ...
                if 'Verifying you are human' in page_content or 'challenges.cloudflare.com' in page_content:
                    if verbose:
                        print("\n[WARN] Hide.mn: Cloudflare challenge re-appeared. Re-solving with browser...")

                    new_creds = _solve_challenge_and_get_creds(sb, url, verbose, turnstile_delay)
                    if not new_creds:
                        print("\n[ERROR] Hide.mn: Failed to re-solve challenge. Aborting.")
                        break
                ...
                if "No proxies found" in page_content:
                    if verbose: print(f"\n[INFO] Hide.mn: Page reports no more proxies.")
                    break

                newly_found = extract_proxies_from_content(page_content, verbose=False)
                if not newly_found:
                    if verbose: print(f"\n[INFO]   ... No proxies found on this page. Assuming end of list.")
                    break
                ...
                if verbose:
                    print(f"\n[INFO]   ... Hide.mn: Found {len(newly_found)} proxies. Total unique: {len(all_proxies)}.")

                if len(all_proxies) == initial_count:
                    if verbose: print("\n[INFO]   ... Hide.mn: No new unique proxies found. Stopping.")
                    break
                ...
                if verbose:
                    print(f"\n[ERROR] Hide.mn: Request failed for offset {offset}: {e}. Stopping.")
                break

    except Exception as e:
        if verbose:
            print(f"\n[ERROR] A critical exception occurred in Hide.mn scraper: {e}")

    if verbose:
        print(f"\n[INFO] Hide.mn: Finished. Found a total of {len(all_proxies)} unique proxies.")
